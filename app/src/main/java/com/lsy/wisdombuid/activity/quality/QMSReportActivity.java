...
//    private RectNoticeAdapter listAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        sharedUtils = new SharedUtils(QMSReportActivity.this, SharedUtils.WISDOM);
...
//        //===
        ...
        listAdapter = new UpdatePictureAdapter(QMSReportActivity.this, picPahts);
    ...
    private void initData() {
        ...
        if (data != "" || !data.equals("")) {
            ...
            rName.setText("" + recordData.getQuality_name());
        } else {
            ToastUtils.showBottomToast(QMSReportActivity.this, "获取数据失败");
        }
...
//        id(安全隐患id),zg_url1(地址数组),zg_description(整改说明)
        listcanshu.put("id", recordData.getQuality_id());
        listcanshu.put("zg_url1", "" + imageList.toString());
        listcanshu.put("section_id", OKHttpClass.getToken(QMSReportActivity.this));//标段ID登录返回
        listcanshu.put("zg_description", rContent.getText().toString());//(整改说明)

        //设置请求类型、地址和参数
        okHttpClass.setPostCanShu(QMSReportActivity.this, RequestURL.updateZgUrl, listcanshu);
        okHttpClass.setGetIntenetData(new OKHttpClass.GetData() {
            @Override
            public String requestData(String dataString) {
                //请求成功数据回调
                L.log("safetyInspectionRecord", "record=待整改updateZgUrl=" + dataString);
                ...
                try {
                    ...
                    if (code == 200) {
                        ...
                        ToastUtils.showBottomToast(QMSReportActivity.this, "" + message);
                    }
    ...
    private boolean is_input() {
        ...
        if (rContent.getText().toString().length() < 1) {
            Toast.makeText(QMSReportActivity.this, "请填写整改内容", Toast.LENGTH_SHORT).show();
            return false;
        }

        if (imageList.size() < 1) {
            Toast.makeText(QMSReportActivity.this, "请上传现场整改图片", Toast.LENGTH_SHORT).show();
            return false;
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, @androidx.annotation.Nullable Intent data) {
        ...
        if (requestCode == 123) {
            ...
            Log.d("拍照返真实:", RealPathFromUriUtils.compressImage(photoPath, QMSReportActivity.this));
//            picPahts.add(0, photoPath);

        } else if (requestCode == 234) {
            if (null != data && data.getData() != null) {
                photoPath = RealPathFromUriUtils.getRealPathFromUri(QMSReportActivity.this, data.getData());
                uri = data.getData();
...
//        horListPhoto.setAdapter(horPhotoAdapter);
    ...
    private void updateOss(String filename, String filePath) {
        ...
        OssService ossService = new OssService(QMSReportActivity.this, "LTAI4Fjcn7J9c5aCVFTYabqE", "EuufkpKHommuLDd6EawJQac8togdPn", "http://oss-cn-shanghai.aliyuncs.com", "jjjt");
        //初始化OSSClient
