...
import com.lsy.wisdombuid.adapter.QualityRecordAdapter;
 ...
 * todo : 质量检查记录
 */
public class QMSRecordActivity extends MyBaseActivity {

    //消息列表
    private RecyclerView idListRecycle;
    private QualityRecordAdapter listAdapter;
...
//        {"pageNo":1,"pageSize":10,"section_id":2}
        ...
        listcanshu.put("section_id", OKHttpClass.getToken(QMSRecordActivity.this));//标段ID登录返回
        listcanshu.put("station_id", station_id);//站点ID(下拉框里面选的)

        L.log("record", "station_id===" + OKHttpClass.getToken(QMSRecordActivity.this));

        //设置请求类型、地址和参数
        okHttpClass.setPostCanShu(QMSRecordActivity.this, RequestURL.findQualityshow_JCJL, listcanshu);
        okHttpClass.setGetIntenetData(new OKHttpClass.GetData() {
            @Override
            public String requestData(String dataString) {
                //请求成功数据回调
                L.log("safetyInspectionRecord", "findQualityshow_JCJL==" + dataString);
                Gson gson = new Gson();
                ...
                try {
                    ...
                    if (dataList != null && dataList.size() > 0) {
                        listAdapter = new QualityRecordAdapter(QMSRecordActivity.this, dataList, 1);
...
//        {"pageNo":1,"pageSize":10,"section_id":2}
        ...
        //设置请求类型、地址和参数
        okHttpClass.setPostCanShu(QMSRecordActivity.this, RequestURL.findQualityshow_WXJL, listcanshu);
        okHttpClass.setGetIntenetData(new OKHttpClass.GetData() {
            @Override
            public String requestData(String dataString) {
                //请求成功数据回调
                L.log("record", "无效数据=findQualityshow_WXJL=" + dataString);
                Gson gson = new Gson();
                ...
                try {
                    ...
                    if (dataList != null && dataList.size() > 0) {
                        listAdapter = new QualityRecordAdapter(QMSRecordActivity.this, dataList, 2);
                        idListRecycle.setAdapter(listAdapter);
                        noData.setVisibility(View.GONE);
                    } else {
                        noData.setVisibility(View.VISIBLE);
    ...
    private void initView() {
        ...
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(QMSRecordActivity.this);
